...
int main(void)
{
	...
	// 0 .
	LATB = 0xF000; //turns off all the LEDs
	...
	// Assign the TRISB bit for this pin to configure this port as an input.
        TRISBbits.TRISB5  = 1;  //sets the switch to be input
	...
	while(1)
	{
		...
		// the initially defined rate.
            ...
            switch(state) {
                case 0:                             //Starts in State 0, which is when no button is being pressed
                                                    //Waits for the button to be pressed
                    PR1 = 14400;                    //Sets the timer to be 250ms
                    if(PORTBbits.RB5 == 0)          //If the button is pressed, change states
                        state = 1;
                    break;
                case 1:                             //Recognizes the button is pressed and changes to next state
                    PR1 = 7200;                     //Sets the timer to be 125ms
                    TMR1 = 0;                       //Resets the timer so there is no delay
                    state = 2;
                    break;
                case 2:                             //Waits for the button to no longer be pressed and then goes back to state 0
                    if (PORTBbits.RB5 == 1)
                        state = 0;
                    break;
            }
		...
		if(IFS0bits.U1RXIF == 1) {	
			...
			if( receivedChar <= '7' && receivedChar >= '4' ) {
				...
				printf("Toggling LED%d\n\r", ledToToggle);
                                LATB = 0xF000;          //turns off all the LEDs incase some were still on after switching
			}
			else {
